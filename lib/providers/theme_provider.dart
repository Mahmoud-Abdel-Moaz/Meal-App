...
class ThemeProvider with ChangeNotifier {
  ...
  getThemeColors() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    primaryColor =
        _setMaterialColor(prefs.getInt("primaryColor") ?? Colors.pink.value);
    accentColor =
        _setMaterialColor(prefs.getInt("accentColor") ?? Colors.amber.value);
    notifyListeners();
  }
  ...
  getThemeMode() async {
    SharedPreferences prefs = await SharedPreferences.getInstance();
    themeText = prefs.get("themeText") ?? "s";
    if (themeText == "d")
      tm = ThemeMode.dark;
    else if (themeText == "l")
      tm = ThemeMode.light;
